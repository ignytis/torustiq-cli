...
Pipeline::Pipeline(const PipelineDefinition& def, HostGlobals globals) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        Stages::AbstractStage* stage =
            (i == 0) ? static_cast<Stages::AbstractStage*>(
                           new Stages::SourceStage(stageDef, globals))
            : (i == count - 1)
                ? static_cast<Stages::AbstractStage*>(
                      new Stages::SinkStage(stageDef, globals))
                : static_cast<Stages::AbstractStage*>(
                      new Stages::ProcessorStage(stageDef, globals));
